...
impl From<String> for HFSString {
    fn from(str: String) -> Self {
        HFSString(str.nfd().collect::<String>().encode_utf16().collect())
    }
}
        ...
        for idx in 0..num_offsets {
            ...
            if offset < 14 || offset > last_offset_pos {
                return Err(Error::InvalidData(
                    "Invalid record offset value".to_string(),
                ));
            }
            offsets.push(offset);
        }
...
impl<F: Read + Seek, K: Key, R: Record<K>> BTree<F, K, R> {
    pub fn open(mut fork: F) -> Result<BTree<F, K, R>> {
        ...
        fork.read_exact(&mut buffer)
            .expect("Failed to read from fork");
        let node_size = (&buffer[32..34])
            .read_u16::<BigEndian>()
            .expect("Error decoding node size");
        let remaining = node_size - buffer.len() as u16;
        for _ in 0..remaining {
            buffer.push(0);
        }
        fork.read_exact(&mut buffer[512..])
            .expect("Failed to read from fork");
    ...
    pub fn get_record_range(&mut self, first: &K, last: &K) -> Result<Vec<Rc<R>>> {
        self.get_record_range_node(first, last, self.header.header.rootNode as usize)
    }
}

pub type BTreeRc<F, K, R> = Rc<RefCell<BTree<F, K, R>>>;
    ...
    pub fn load(
        ...
        while let Some(extent_list) = extents_result {
            for extent in &extent_list {
                ...
                let extent_position_clamped = std::cmp::min(data.logicalSize, extent_position);
                let extent_end_clamped = std::cmp::min(data.logicalSize, extent_end);
                // println!("{} = {} = {} = {}", extent.startBlock, extent.blockCount, extent_position_clamped, extent_end_clamped);
                extents.push((
                    extent.startBlock,
                    extent.blockCount,
...
pub type ForkRc<F> = Rc<RefCell<Fork<F>>>;
...
impl Record<CatalogKey> for CatalogRecord {
    fn import(source: &mut dyn Read, key: CatalogKey) -> Result<Self> {
        ...
        let body = match record_type {
            ...
            internal::kHFSPlusFileRecord => CatalogBody::File(HFSPlusCatalogFile::import(source)?),
...
impl<F: Read + Seek> HFSVolume<F> {
    pub fn load(mut file: F) -> io::Result<Rc<RefCell<HFSVolume<F>>>> {
        ...
        let catalog_fork = Fork::load(
            Rc::clone(&volume.borrow().file),
            kHFSCatalogFileID,
            0,
            Rc::clone(&volume),
            &volume.borrow().header.catalogFile,
        )?;
        let extents_fork = Fork::load(
            ...
            0,
            Rc::clone(&volume),
            &volume.borrow().header.extentsFile,
        )?;
    ...
    pub fn get_children_id(&self, node_id: HFSCatalogNodeID) -> Result<Vec<Rc<CatalogRecord>>> {
        let mut btree = self
            ...
            .expect("Can't unwrap")
            .borrow_mut();
        let first = CatalogKey {
            _case_match: false,
            parent_id: node_id,
            node_name: HFSString::from(""),
        };
        let last = CatalogKey {
            _case_match: false,
            parent_id: node_id + 1,
    ...
    pub fn get_children(&self, key: &CatalogKey) -> Result<Vec<Rc<CatalogRecord>>> {
        let record = {
    ...
    pub fn get_path_record(&self, filename: &str) -> Result<CatalogRecord> {
        ...
        let root_thread_key = CatalogKey {
            _case_match: false,
            parent_id: 2,
            node_name: HFSString::from(""),
        };
        ...
        let thread = match result.body {
            CatalogBody::FolderThread(ref x) => x,
            _ => {
                return Err(Error::InvalidRecordType);
            }
        };
        ...
        Ok(CatalogRecord {
            key: result.key.clone(),
            body: CatalogBody::Folder(parent),
        })
    }
